...
export default function Project({ project }) {
  return (
    <div id={project.tag} className="overflow-hidden bg-white py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 bg-white">
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-12 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
          <div className="lg:pr-8 lg:pt-4">
            <div className="lg:max-w-lg">
              <p className="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl mb-2">{project.name}</p>
              {
                project.stack.map((technology, index) => (
                  <span
                    key={index}
                    className={`inline-flex items-center rounded-md bg-gray-50 px-2 py-1 text-xs font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 ${
                      index < project.stack.length - 1 ? 'mr-1' : '' // Add margin to the right for all elements except the last one
                    }`}
                  >
                    {technology}
                  </span>
